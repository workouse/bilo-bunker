  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-dark-muted">Real-time status of your isolated Durable Object Bunker instance</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="glass-card p-5 rounded-xl space-y-2">
          ...
          <span className="text-xs text-indigo-400">SQLite DO Isolated</span>
        </div>

        <div className="glass-card p-5 rounded-xl space-y-2">
          <div className="flex items-center justify-between text-dark-muted">
            <span className="text-xs font-semibold uppercase tracking-wider">Edge Topology</span>
            <Cpu className="w-5 h-5 text-accent-amber" />
          </div>
          <p className="text-3xl font-bold text-white">Cloudflare</p>
          <span className="text-xs text-amber-400">Durable Object Engine</span>
        </div>
      </div>
      ...
      <div className="glass-card p-6 rounded-xl space-y-4 border border-dark-border">
        ...
        <p className="text-sm text-dark-muted leading-relaxed">
          Your Nostr private keys are statefully stored inside an isolated Cloudflare Durable Object equipped with an embedded SQLite engine.
          No raw private keys ever leave the edge worker sandbox. All remote client requests are authenticated via NIP-46, encrypted with NIP-44 v2, and audited in your real-time log stream.
        </p>
      </div>
    </div>
  );
